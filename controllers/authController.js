...
exports.signupUser = asyncHandler(async function (req, res) {
  ...
  sendGeneratedToken(user, res, 201);
});
...
exports.tokenProtect = asyncHandler(async function (req, res, next) {
  ...
  async function checkUser() {
    ...
    if (!currUser) {
      return res.status(404).json({ message: "User is not legged in!" });
    }
...
exports.restrictTo = function (...userRoles) {
  return function (req, res, next) {
    if (!userRoles.includes(req.user.role)) {
      return res
        .status(401)
        .json({ message: "You do not have permision to perform this action!" });
    }
    next();
  };
};
...
exports.forgotPassword = asyncHandler(async function (req, res) {
  ...
  try {
    const resetUrl = (process.env.NODE_ENV = "development"
      ...
      : `https://drmartensns.onrender.com/reset-password/${resetToken}`);
    ...
    return res
      .status(500)
      .json({ message: "There was en error sending email!" });
  }
});
